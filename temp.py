...
def is_stationary_ar(ar: Array1D[float]) -> bool:
    """
    return true if the ar polynomial has roots outside the unit circle
    """
    ar: Final[Array1D[float]] = np.asarray(ar, dtype=float)
    if len(ar) == 0:
        return True
    poly = np.r_[-ar[::-1], 1.0]
    roots = np.roots(poly)
...
def is_invertible_ma(ma: Array1D[float]) -> bool:
    """
    return true if the ma polynomial has roots outside the unit circle
    """
    ma: Final[Array1D[float]] = np.asarray(ma, dtype=float)
    if len(ma) == 0:
        return True
    poly = np.r_[ma[::-1], 1.0]
    roots = np.roots(poly)
...
def simulate_arma(n, ar: Array1D[float] | None = None, ma: Array1D[float] | None = None, sigma: float = 1.0, burn: int = 1000, seed: int | None = None):
    """
    ...
    """
    if ar is None:
        ar: Array1D[float] = np.array([], dtype=float)
    if ma is None:
        ma: Array1D[float] = np.array([], dtype=float)

    ar: Array1D[float] = np.asarray(ar, dtype=float)
    ma: Array1D[float] = np.asarray(ma, dtype=float)
    ...
    x: Final[Array1D[float]] = np.zeros(total, dtype=float)

    for t in range(r, total):
        ar_part: float = 0.0
        if p > 0:
            ar_part: float = np.dot(ar, x[t-p:t][::-1])
...
def empirical_acf(x: Array1D[float], k: int) -> Array1D[float]:
    ...
    xc: Final[Array1D[float]] = x - x.mean()
    denom: Final[float] = np.dot(xc, xc)

    acf: Final[Array1D[float]] = np.empty(k, dtype=float)
...
def pacf_from_acf(rho: Array1D[float]) -> Array1D[float]:
    """
    given rho[0],...,rho[k], return pacf at lags 1,...,k
    using durbin-levinson recursion
    """
    rho: Final[Array1D[float]] = np.asarray(rho, dtype=float)
    k: Final[int] = len(rho) - 1

    if k < 1:
        return np.array([], dtype=float)

    phi: Final[Array2D[float]] = np.zeros((k + 1, k + 1), dtype=float)
    pacf: Final[Array1D[float]] = np.zeros(k + 1, dtype=float)

    phi[1, 1] = rho[1]
    pacf[1] = rho[1]

    v: float = max(1.0 - rho[1] ** 2, 1.0e-12)
    ...
    for m in range(2, k + 1):
        ...
        v: float = max(v * (1.0 - phi[m, m] ** 2), 1.0e-12)

    return pacf[1:]


def empirical_pacf(x: Array1D[float], k: int):
    """
    sample pacf at lags 1,...,k
    """
    acf = empirical_acf(x, k=k)
    rho: Final[Array1D[float]] = np.empty(k + 1, dtype=float)
    rho[0] = 1.0
    rho[1:] = acf
    return pacf_from_acf(rho)


def impulse_response_arma(ar: Array1D[float], ma: Array1D[float], npsi: int) -> Array1D[float]:
    """
    ...
    psi: Final[Array1D[float]] = np.zeros(npsi, dtype=float)
    psi[0] = 1.0
...
def theoretical_acf_arma(ar: Array1D[float], ma: Array1D[float], k: int, npsi: int | None = None):
    """
    ...
    if npsi is None:
        npsi: Final[int] = max(500, 50 * (k + 1), 200 * (max(len(ar), len(ma)) + 1))

    psi = impulse_response_arma(ar, ma, npsi=npsi)

    gamma: Final[Array1D[float]] = np.empty(k + 1, dtype=float)
    for h in range(k + 1):
...
def theoretical_pacf_arma(ar: Array1D[float], ma: Array1D[float], k: int, npsi: int | None = None):
    """
...
def yule_walker_ar_start(acf_emp: Array1D[float], p: int) -> Array1D[float]:
    """
    ...
    rho: Final[Array1D[float]] = np.empty(p + 1, dtype=float)
    rho[0] = 1.0
    rho[1:] = acf_emp[:p]

    rmat: Final[Array2D[float]] = np.empty((p, p), dtype=float)
    for i in range(p):
        for j in range(p):
...
def split_params(params: Array1D[float], p, q):
    """
    split parameter vector into ar and ma parts
    """
    params: Final[Array1D[float]] = np.asarray(params, dtype=float)
    ar = params[:p]
    ma = params[p:p + q]
    return ar, ma


def arma_match_objective(params: Array1D[float], acf_emp, pacf_emp, p, q, mode: str = "both", pacf_weight: float = 0.5, npsi: int | None = None) -> float:
    """
    ...
    w: Final[Array1D[float]] = 1.0 / np.arange(1, k + 1, dtype=float)
...
def local_coordinate_search(start, acf_emp: str, pacf_emp: float, p: int, q, mode: str = "both", pacf_weight: float = 0.5, step0: float = 0.3, step_min: float = 1.0e-4, shrink: float = 0.5, max_rounds: int = 100, bound: float = 0.98, npsi: int | None = None, rng = None):
    """
...
def fit_arma_initial_numpy(x: Array1D[float], p: int, q: int, k: int, mode: str = "both", pacf_weight: float = 0.5, n_starts: int = 25, step0: float = 0.3, step_min: float = 1.0e-4, shrink: float = 0.5, max_rounds: int = 100, bound: float = 0.98, npsi: int | None = None, seed: int | None = None):
    """
    ...
    starts: Final[Array1D[float]] = []

    ar0 = yule_walker_ar_start(acf_emp, p)
    ma0: Final[Array1D[float]] = np.zeros(q, dtype=float)
...
def compare_values(emp: Array1D[float], theo: Array1D[float]):
    """
    return lag, empirical, theoretical, difference
    """
    emp: Final[Array1D[float]] = np.asarray(emp, dtype=float)
    theo: Final[Array1D[float]] = np.asarray(theo, dtype=float)
    lags: Final[Array1D[float]] = np.arange(1, len(emp) + 1)
    diff: Final[Array1D[float]] = emp - theo
...
def main():
    n: Final[int] = 4000
    ar_true: Final[Array1D[float]] = np.array([0.60, -0.25])
    ma_true: Final[Array1D[float]] = np.array([0.50, 0.20])
    sigma: Final[float] = 1.0
    k: Final[int] = 10
    seed: Final[int] = 123
